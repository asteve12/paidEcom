...
function BlogSidebar() {
  const { restData, loading, error } = useFetch("/api/posts/sideBar?demo=12");
  const categories = restData && restData.postSidebarData.categories;
  const recent = restData && restData.postSidebarData.recent;
  const router = useRouter();
  const query = router.query;

  if (error) {
    return <div>Can not fetch the posts😢</div>;
  }
  ...
  return (
    <>
      ...
      <aside
        className={`sidebar mobile-sidebar col-lg-3 h-auto skeleton-body skel-shop-products ${
          loading ? "" : "loaded"
        }`}
      >
