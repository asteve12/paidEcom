...
function ProductSidebarOne(props) {
  ...
  const { restData, loading, error } = useFetch(
    "/api/products/sideBar?demo=12&featured=true"
  );
  const categories = useMemo(() => {
    let cats = !loading ? restData.shopSidebarData.categories : [];
    let stack = [],
      result = [];
  ...
  return (
    <>
      ...
      <aside
        className={`sidebar-product col-lg-3 mobile-sidebar skeleton-body skel-shop-products ${adClass} ${
          loading ? "" : "loaded"
        }`}
      >
