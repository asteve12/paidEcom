...
function ShopInfinite() {
  ...
  const total = !loading && data.products.total;
  //
  // inner function
  async function getProducts(parametrs, lazy) {
    const req = await fetch(`/api/products/all_products?${parametrs}`);
  ...
  useEffect(() => {
    ...
    const urlParam = qs.stringify({
      ...
      colors: query?.colors ? query.colors : null,
      sizes: query?.sizes ? query.sizes : null,
    ...
    getProducts(urlParam, false);
  }, [query, sortBy]);

  useLayoutEffect(() => {
    !loading && setProducts(data.products.data);
  }, [loading]);
          ...
          <div className="col-lg-9 main-content">
            ...
            >
              <ProductsGrid
                products={products}
                loading={loading}
                perPage={data ? products.length + 3 : 12}
              />
            </ReactScroll>
          </div>
